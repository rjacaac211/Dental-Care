...
def main():
    ...
    # Define paths for models, data, and results
    ...
    saved_model_path = os.path.join(MODELS_DIR, 'oral_disease_saved_model')
    tf.saved_model.save(model, saved_model_path)
    print("Model saved in SavedModel format at:", saved_model_path)

    # Save the model in .h5 format
    h5_model_path = os.path.join(MODELS_DIR, 'oral_disease_model.h5')
    model.save(h5_model_path)
    print("Model saved in HDF5 format as:", h5_model_path)

    # Save training history as CSV
    history_df = pd.DataFrame(history.history)
    history_csv_path = os.path.join(RESULTS_DIR, 'training_history.csv')
    history_df.to_csv(history_csv_path, index=False)
    print("Training history saved as:", history_csv_path)
    ...
    print("Training graphs saved to:", training_plot_path)
    plt.show()
